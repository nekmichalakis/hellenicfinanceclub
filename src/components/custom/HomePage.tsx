  ...
  const activities = [
    {
      ...
      text: "Meetings",
      description: "between HFN alumni",
    },
    {
      icon: <LoaderCircleIcon className={"size-16"} />,
      text: "Events",
      description: "with industry professionals, advisors and academics",
    },
    {
      icon: <CircleDotIcon className={"size-16"} />,
      text: "Workshops",
      description: "for all our members",
    },
  ];
  ...
  return (
    <div>
        ...
        >
          <div className={"space-y-6 max-w-3xl"}>
            ...
            <p className={"text-md font-thin"}>
              Each of our initiatives is strategically designed to advance our
              mission by expanding your network and deepening your industry
              expertise
            </p>
          </div>
          <div className={"grid grid-cols-1 md:grid-cols-3 gap-8 mt-4"}>
            {activities.map((card, index) => (
              <div key={index} className={"flex flex-col items-center gap-4"}>
                {card.icon}
                <p className={"text-lg font-thin"}>{card.text}</p>
      ...
      <div className="flex flex-col bg-white">
        ...
        >
          <h1 className={"text-4xl md:text-5xl font-medium"}>
            The Only Inter-University Finance Network for Greek Students
          </h1>
          <p className={"text-md font-thin"}>
          ...
          <Link to={"/apply"}>
            <Button
              variant={"primary"}
              className={
                "rounded-full p-10 py-8 font-light hover:opacity-60 self-center my-8"
              }
            >
